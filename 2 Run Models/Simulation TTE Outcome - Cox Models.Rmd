...
### Cox Model: First 500 Iterations
...
```{r}
# cox model
mod_cox <- map(sim_data_tte500,
               ~coxph(Surv(outc_yrs, status) ~ pdl1_perc_norm*tx,
      data = .x, 
      weights = iptw_trim97))
```
...
assign(paste0("mod_cox_scenario_", scenario), mod_cox)
```

```{r}
# save Cox model results
save(list = ls(pattern = "mod_cox_scenario"),
     file = here::here(paste0("data/Project 1/Simulation/Scenario ", scenario, "/scenario_", scenario, "_sim_data_tte_cox_results.rdata")))
```